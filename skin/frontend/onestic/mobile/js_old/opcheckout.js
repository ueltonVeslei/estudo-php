...
Checkout.prototype = {
    ...
    {
        //alert("function go section:"+section+"---------");
    ...
    setMethod: function(method){
    	...
    	if(method && method == "guest"){
            ...
            //Element.hide('register-customer-password'); mudar para jquery dps. aplicar classe no form
            accordion.closeSection("step-login");
...
//        //this.accordion.openNextSection(true);
    ...
    setShippingMethod: function() {
        this.reloadProgressBlock();
        //this.nextStep();
        this.gotoSection('asdasdasdasdas');
        //this.accordion.openNextSection(true);
    },
    ...
    setStepResponse: function(response){
		...
		if (response.goto_shipping) {
			//new Ajax.Updater('shipping-address', response.shipping_update_url);
			new Ajax.Request(response.shipping_update_url, {
				onLoading: function() {
				...
				onSuccess: function(transport) {
					...
					if (response.logged) {
						...
						//$('div-botoes-com-endereco').show();
						//$('div-botoes-sem-endereco').hide();
						$('add-button').show();
						$('add-button-cancel').hide();
						$('save-button').hide();
						$('h4-titulo').update('Escolher endereço de entrega');
						$('shipping-address').update(transport.responseText);
					} else {
						$('shipping-buttons-container').hide();
						$('shipping-method-please-wait').hide();
						$('h4-titulo').update('Endereço de entrega');
						$('co-shipping-method-form').show();
						$('shipping-buttons-container').show();
						$('checkout-shipping-method-load').update(response.html_shipping_method);
						$('onepage-checkout-shipping-method-additional-load').update(response.html_additional);	
					}
					
					/*new Ajax.Request(response.shipping_update_url.replace('updateShipping', 'shippingMethod'), {
						onLoading: function() {
							$('shipping-method-please-wait').show()
						},
						onSuccess: function(transport) {
							...
							$('content-shipping-method').update(transport.responseText);
						}
					});*/
				}
			});
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    nextStep: function(transport){
        ...
        if (response.error){
			...
			//alert(this.saveUrl);
			this.saveUrl = this.saveUrl.replace('saveBilling', 'saveShipping');
			//alert(this.saveUrl);
