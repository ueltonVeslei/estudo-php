<?php
class Av5_LogViewer_Adminhtml_ReportController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function viewAction() {
        $this->getRequest()->setParam('type','report');
        $this->loadLayout();
        $this->_title($this->__("Av5 Log Viewer"));
        $this->renderLayout();
    }
    ...
    public function postAction() {
		...
		try {
			...
			$message='';
			if($log){
				$fp = fopen(Mage::getBaseDir('var') . '/report/' . $log, "r+");
				ftruncate($fp, 0);
				fclose($fp);
	...
	public function deleteAction() {
		...
		try {
			...
			$message='';
			if($log){
				unlink(Mage::getBaseDir('var') . '/report/' . $log);
	...
	function downloadAction($file_name) {
	    ...
	    $file = Mage::getBaseDir('var').'/report/'.$file_name;
