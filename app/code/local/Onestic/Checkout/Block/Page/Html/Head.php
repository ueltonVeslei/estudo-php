...
class Onestic_Checkout_Block_Page_Html_Head extends Mage_Page_Block_Html_Head {
	...
	public function getWhitelistedCssJsItems() {
		$default = array(
			...
			// Checkout
			'onestic/checkout/jquery-1.7.2.min.js',
			'onestic/checkout/jquery-ui-1.8.20.custom.min.js',
			'onestic/checkout/jquery.maskedinput-1.3.min.js',
			'onestic/checkout/jquery.validate.min.js',
			'onestic/checkout/jquery.validate.creditcard2-1.0.1.js',
			'onestic/checkout/opcheckout.js',
			'onestic/checkout/js/validation.js',
			'onestic/checkout/ui-lightness/jquery-ui-1.8.20.custom.css',
			'onestic/checkout/bootstrap/css/bootstrap.min.css',
			'onestic/checkout/css/style.css',
			'css/mundipagg.css',
			'css/bomparatodos.css',
			'css/vidaclass.css',
			'css/mobile_vidaclass.css',
			'uecommerce/mundipagg.js'
		);

		$userDefinedWhitelist = explode( "\n", Mage::getStoreConfig( 'checkout/advanced/whitelisted_css_js' ) );
	...
	public function getCssJsHtml() {
		if ( $this->getRequest()->getModuleName() == 'onestic_checkout' && $this->getRequest()->getControllerName() == 'onepage' && $this->getRequest()->getActionName() != 'success' ) {
			$whitelisted = $this->getWhitelistedCssJsItems();
			foreach ( $this->_data[ 'items' ] as $item ) {
				if ( in_array( $item[ 'name' ], $whitelisted ) ) {
					if ($item['name'] == 'onestic/checkout/js/validation.js') {
						$item['name'] .= '?v=' . filemtime(Mage::getBaseDir() . '/skin/frontend/base/default/' . $item['name']);
					}
