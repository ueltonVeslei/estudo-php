 ...
 * @package     Checkout
 * @category    Onestic
 * @copyright   Copyright 2012 Onestic (http://www.Onestic.com)
 * @version:    0.0.2
 */
class GeoIP_Record {
...
class Onestic_Checkout_Model_GeoIP_City {
	...
	static function getrecordwithdnsservice( $str ) {
		...
		$number = Onestic_Checkout_Model_GeoIP_Core::$GEOIP_COUNTRY_CODE_TO_NUMBER[$record->country_code];
		$record->country_code3 = Onestic_Checkout_Model_GeoIP_Core::$GEOIP_COUNTRY_CODES3[$number];
		$record->country_name = Onestic_Checkout_Model_GeoIP_Core::$GEOIP_COUNTRY_NAMES[$number];
		if ( $record->region != "" ) {
	...
	static function _get_record( $gi, $ipnum, Onestic_Checkout_Model_GeoIP_Core $geoip ) {
		...
		if ( $gi->flags & Onestic_Checkout_Model_GeoIP_Core::GEOIP_MEMORY_CACHE ) {
			$record_buf = substr( $gi->memory_buffer, $record_pointer, self::FULL_RECORD_LENGTH );
		} elseif ( $gi->flags & Onestic_Checkout_Model_GeoIP_Core::GEOIP_SHARED_MEMORY ) {
		...
		if ( Onestic_Checkout_Model_GeoIP_Core::GEOIP_CITY_EDITION_REV1 == $gi->databaseType ) {
			$metroarea_combo = 0;
	...
	static function GeoIP_record_by_addr( $gi, $addr, Onestic_Checkout_Model_GeoIP_Core $geoip ) {
		if ( $addr == NULL ) {
			return 0;
		}
		$ipnum = ip2long( $addr );
