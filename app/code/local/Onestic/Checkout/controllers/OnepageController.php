...
class Onestic_Checkout_OnepageController extends Mage_Checkout_Controller_Action {
	...
	public function preDispatch() {
		...
		if ( $this->getRequest()->getActionName() != 'success' ) {
			Mage::helper( 'onestic_checkout' )->estimateShipping();
		}
	...
	protected function _getShippingMethodsHtml() {
		...
		if ( Mage::helper( 'onestic_checkout' )->isVirtualOnly() ) {
			$update->load( array( 'checkout_onepage_shippingmethod', 'onestic_checkout_virtual' ) );
		} else {
	...
	protected function _getPaymentMethodsHtml() {
		...
		$update = $layout->getUpdate();
		if ( Mage::helper( 'onestic_checkout' )->isVirtualOnly() ) {
			$update->load( array( 'onestic_checkout_onepage_paymentmethod', 'onestic_checkout_virtual' ) );
		} else {
			$update->load( 'onestic_checkout_onepage_paymentmethod' );
		}
		$layout->generateXml();
		$layout->generateBlocks();
		$output = $layout->getOutput();
	...
	protected function _getAdditionalHtml() {
		...
		$update = $layout->getUpdate();
		if ( Mage::helper( 'onestic_checkout' )->isVirtualOnly() ) {
			$update->load( array( 'checkout_onepage_additional', 'onestic_checkout_virtual' ) );
		} else {
	...
	public function indexAction() {
	    if (Mage::app()->getStore()->getId() == 1) {
            $dscUrl = Mage::helper('fd_linkcheckout')->getCheckoutLink();
            header('location: ' . $dscUrl);
            return;
        }
		...
		if (!Mage::helper('customer')->isLoggedIn()) {
			...
			Mage::getSingleton('customer/session')->setOnCheckout(true);
	...
	public function getAddressAction() {
		...
		if ( $addressId ) {
			...
			if ( Mage::getSingleton( 'customer/session' )->getCustomer()->getId() == $address->getCustomerId() ) {
				...
				$address->setFullname( Mage::helper( 'onestic_checkout' )->getFullname( $address ) );
	...
	public function saveBillingAction() {
		...
		if ( $this->getRequest()->isPost() ) {
			...
			$data = Mage::helper( 'onestic_checkout' )->prepareAddressData( $data );
			...
			if(isset($data['country_id'])){
				if(Mage::helper('directory')->isRegionRequired($data['country_id'])
					...
					$result['message'][] = Mage::helper('onestic_checkout')->__('"%s" is a required value.', $region_model->getStoreLabel());
				}
			}
	...
	public function saveShippingMethodAction() {
		...
		if ( $this->getRequest()->isPost() ) {
			...
			$shipping = Mage::helper( 'onestic_checkout' )->prepareAddressData( $shipping );
			$this->getOnepage()->getQuote()->setTotalsCollectedFlag(true);
			if(!Mage::getStoreConfig( 'checkout/options/separate_shipping_method_step' )){
			...
			if ( Mage::getSingleton( 'customer/session' )->isLoggedIn()
				...
				Mage::helper( 'onestic_checkout' )->addCustomerPassword( $shipping, $password );
	...
	public function savePaymentAction() {
		...
		try {
			...
			if ( !array_key_exists( 'same_as_shipping', $billing ) && !$this->getOnepage()->getQuote()->isVirtual() ) {
				$billing = Mage::helper( 'onestic_checkout' )->prepareAddressData( $billing );
				if ( !Mage::getSingleton( 'customer/session' )->isLoggedIn() ) {
					$billing['email'] = $this->getOnepage()->getQuote()->getBillingAddress()->getEmail();
					if( $password ) {
						Mage::helper( 'onestic_checkout' )->addCustomerPassword( $billing, $password );
					} else {
						Mage::helper( 'onestic_checkout' )->addCustomerPassword( $billing );
					}
				}
				...
				if( $password ) {
					Mage::helper( 'onestic_checkout' )->addCustomerPassword( $shipping_address, $password );
				} else {
					Mage::helper( 'onestic_checkout' )->addCustomerPassword( $shipping_address );
				}
	...
	public function saveOrderAction() {
		...
		try {
			...
			$gateway_error_message = Mage::getStoreConfig( 'checkout/options/gateway_error_message' );
			...
			if ( ! empty( $gateway_error_message ) ) {
				$result['error_messages'] .= "<br /><br />" . Mage::getStoreConfig( 'checkout/options/gateway_error_message' );
			...
			foreach ( $address_components as $address_component ) {
				foreach ( $address_component->types as $type ) {
					if ($type == "sublocality") {
						...
						$result->error = false;
						$countryOption = Mage::helper( 'onestic_checkout' )->getAllowedCountryOptionById( $address_component->short_name );
