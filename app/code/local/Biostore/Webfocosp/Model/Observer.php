<?php
class Biostore_Webfocosp_Model_Observer
{
    /**
    * This action will generate webfocosp.xml to save to <website>/var/webfocosp/
    * @return void
    */
    public function sendProductTowebfocosp()
    {
        $send_to_webfocosp = Mage::getStoreConfig('catalog/webfocosp/send_to_webfocosp');
        $webfocosp_parcelomento = Mage::getStoreConfig('catalog/webfocosp/webfocosp_parcelomento');
        $product_suffix_url = Mage::getStoreConfig('catalog/webfocosp/product_suffix_url');
        ...
        /*
        if ($send_to_webfocosp == 0) {
            $collection = Mage::getModel('catalog/product')->getCollection()
                ->addAttributeToFilter('enviar_para_webfocosp', array('eq' => 1))
        ...
        if ($send_to_webfocosp == 0) {
            ...
            $query = "SELECT CPF1.* FROM catalog_product_flat_1 AS CPF1
                        ...
                        WHERE EA.attribute_code = 'enviar_para_webfocosp' AND CPEI.value = 1 AND visibility = 4";
        ...
        if (count($products)) {
            foreach ($products as $product) {
                ...
                if ('bragspag' == $webfocosp_parcelomento) {
                    $maximo = Mage::getModel('braspagcc/payment_gateway')->getParcelamentoMaximo2($price);
                } else if ('parcelamento' == $webfocosp_parcelomento) {
        ...
        $path_to_save_xml = Mage::getBaseDir().'/webfocosp/';
        if(is_dir($path_to_save_xml)){
            $doc->save($path_to_save_xml.'webfocosp.xml');
        }
        else {
