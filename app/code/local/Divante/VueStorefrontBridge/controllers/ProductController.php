...
class Divante_VueStorefrontBridge_ProductController extends Divante_VueStorefrontBridge_AbstractController
{
    public function indexAction()
    {
        if($this->_authorize($this->getRequest())) {
            ...
            $productUrl = explode('/', $this->getRequest()->getParam('url'));
            $productUrl = end($productUrl);

            $result = array();
            $productCollection = Mage::getModel('catalog/product')
                ->getCollection()
                ->addAttributeToSelect('*')
                ->addFieldToFilter('url_path', $productUrl);
