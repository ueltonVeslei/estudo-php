...
class AW_Collpur_Block_Customer_Basedeal extends Mage_Core_Block_Template {
    ...
    public function getBackUrl() {
     ...
     public function getPurchaseStatus($orderItemId) {
        ...
        if(!$orderItem->getId()) return 'Status Desconhecido';
        if(!(int) $orderItem->getQtyInvoiced() || $orderItem->getQtyInvoiced() != $orderItem->getQtyOrdered()) return 'Não Pago';
        if((int) $orderItem->getQtyInvoiced() && $orderItem->getQtyInvoiced() == $orderItem->getQtyRefunded()) return 'Reembolsado';
        if($orderItem->getQtyInvoiced() == $orderItem->getQtyOrdered()) return 'Pago';
