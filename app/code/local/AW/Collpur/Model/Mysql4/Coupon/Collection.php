...
class AW_Collpur_Model_Mysql4_Coupon_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
    ...
    public function _construct() {
        ...
        $this->_map['fields']['statuses'] = "
                   ...
                   CASE
                        WHEN `main_table`.`status`= 'used' THEN 'Usado'
                        WHEN `main_table`.`status` = 'pending' AND `deals`.`close_state`= " . AW_Collpur_Model_Deal::STATE_OPEN . " AND deals.available_to < UTC_TIMESTAMP() THEN 'Expirado'
                        WHEN `main_table`.`status` = 'pending' AND `deals`.`close_state`= " . AW_Collpur_Model_Deal::STATE_CLOSED . " THEN 'Expirado'
                        WHEN `main_table`.`status` = 'not_used' AND IF((`deals`.`coupon_expire_after_days` = 0 OR `deals`.`coupon_expire_after_days` is null) OR `deals`.`available_to` IS NULL,false, UTC_TIMESTAMP() > DATE_SUB(deals.available_to,INTERVAL -`deals`.`coupon_expire_after_days` DAY)) THEN 'Expirado'
                        WHEN `main_table`.`status` = 'not_used' THEN 'Não Usado'
                        WHEN `main_table`.`status` = 'pending' THEN 'Pendente'
                        WHEN `main_table`.`status` = 'expired' THEN 'Expirado'
                    END
        ";
    }
