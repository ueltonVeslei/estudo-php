...
class AW_Collpur_Helper_Deals extends Mage_Core_Helper_Abstract 
{
    ...
    public function getTimeLeftToBuy($deal=false, $time=false, $now=false, $gmtTo=false) {
        ...
        if($amount < 86400): // Se o tempo restante for menor que 1 dia, mostra os segundos - STeV
            $block = '<span class="horas">%s</span><span class="minutos">%s</span><span class="segundos">%s</span>';
            return sprintf($block, $hours,$minutes,$seconds);
        else: // Se for maior que 1 dia mostra os dias restantes - STeV
            $block = '<span class="dias">%s</span><span class="horas">%s</span><span class="minutos">%s</span>';
            return sprintf($block, "{$days} $daysPHP",$hours,$minutes);
        endif;
