...
class AW_Zblocks_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _isValid($data)
    {
        ...
        switch ($data['schedule_pattern'])
        {
            ...
            case '30'          :
            case '31'          :
                if ((string)$time['mday'] !== $data['schedule_pattern']) return false;
                break;
            case '1,11,21'     :
                if (($time['mday'] % 10) - 1 || $time['mday'] == '31') return false;
                break;
            case '1,11,21,31'  :
                if (($time['mday'] % 10) - 1) return false;
                break;
            case '10,20,30'    :
                if ($time['mday'] % 10) return false;
                break;
            case 'mo'          :
            case 'tu'          :
            case 'we'          :
            ...
            case 'sa'          :
            case 'su'          :
                if (strtolower(substr($time['weekday'], 0, 2)) !== $data['schedule_pattern']) return false;
                break;
            case 'mon-fri'     :
                if (!$time['wday'] || $time['wday'] == 6) return false;
                break;
            case 'sat-sun'     :
                if ($time['wday'] !== 0 && $time['wday'] !== 6) return false;
                break;
            case 'tue-fri'     :
                if ($time['wday'] < 2 || $time['wday'] > 5) return false;
                break;
            case 'mon-sat'     :
                if (!$time['wday']) return false;
                break;
            case 'mon,wed,fri' :
                if ($time['wday'] !== 1 && $time['wday'] !== 3 && $time['wday'] !== 5) return false;
                break;
            case 'tue,thu,sat' :
                if ($time['wday'] !== 2 && $time['wday'] !== 4 && $time['wday'] !== 6) return false;
                break;
            default :
    ...
    public function getBlocks($customPosition, $blockPosition, $categoryPath, $currentCategoryId)
    {
        ...
        if (in_array($blockPosition, Mage::getSingleton('zblocks/source_position')->getNeedCategoryPositions())) {
