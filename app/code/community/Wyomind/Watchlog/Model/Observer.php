...
class Wyomind_Watchlog_Model_Observer
{
    ...
    public function purgeData()
    {
        ...
        $historyLength = Mage::getStoreConfig('watchlogpro/settings/history');
        $deleteBefore = $timestamp - $historyLength * 60 * 60 * 24;
        
        if ($historyLength != 0) {
            $rowBeforeDate = Mage::getSingleton('core/date')->gmtDate('Y-m-d H:i:s', $deleteBefore);
            $log = array();
            $log[] = "-------------------- PURGE PROCESS --------------------";
            $log[] = "current date : " . Mage::getSingleton('core/date')->gmtDate('Y-m-d H:i:s', $timestamp);
            $log[] = "deleting row before : " . $rowBeforeDate;

            $resource = Mage::getSingleton('core/resource');
            $watchlog = $resource->getTableName('watchlog');
            $writeConnection = $resource->getConnection('core_write');
            $writeConnection->delete($watchlog, 'date < "' . $rowBeforeDate . '"');
            
            $log[] = "DELETE FROM " . $watchlog . " WHERE date < '" . $rowBeforeDate . "'";
                    ...
                    foreach ($cronExpr->hours as $h) {
                        ...
                        if ($cron['tasks'][$i]['localTime'] >= $cron['file']['localTime'] && $cron['tasks'][$i]['localTime'] <= $cron['curent']['localTime'] && $done != true) {
                            ...
                            $date = Mage::getSingleton('core/date')->gmtDate(
                                'Y-m-d H:i:s', 
                                Mage::getSingleton('core/date')->gmtTimestamp() - Mage::getStoreConfig('watchlogpro/settings/report_period') * 86400
                            );

                            $history = Mage::getResourceModel('watchlog/watchlog')->getSummary();
                            $history->addFieldToFilter('date', array('gteq' => $date));
