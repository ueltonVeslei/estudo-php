...
class Adyen_Payment_Helper_Installments
{
    ...
    public function getInstallmentForCreditCardForProduct($ccType,$amount) {
        ...
        for($i=1;$i<=$max_installments;$i++){
            ...
            $partial_amount_formatted = Mage::helper('core')->currency($partial_amount, true, false);

            if(isset($installment[3]) && $installment[3] > 0) {
                $result = $result . '<p class="com-juros"><span>'. $i. 'x com juros<span> <strong>'. $partial_amount_formatted. '</strong></p>';
            } else {
                $result = $result . '<p class="sem-juros"><span>'. $i. 'x sem juros<span> <strong>'. $partial_amount_formatted. '</strong></p>';
            }
