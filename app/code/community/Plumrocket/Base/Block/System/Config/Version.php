...
class Plumrocket_Base_Block_System_Config_Version extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $mvd = strtolower($this->getModuleName()) . '_' . Mage::getModel('core/date')->gmtDate('m_d');

        if (($mcache = Mage::app()->getCache()->load($mvd))) {
            ...
            if (!empty($message) && !empty($version)) {
                Mage::app()->getCache()->save(serialize($mcache), $mvd);
            }
        }
        ...
        if (!empty($message) && !empty($version)) {
            if (version_compare($version, (string)$m->version, '>')) {
                $messageHtml = "<script>
                     var messageBlock = $('messages'),
                     messageText = '" . $message . "';
                     if (messageBlock) {
                         messageBlock.insert('<ul class=\'messages\'><li class=\'notice-msg\'><ul><li>'
                             + messageText
                             + '</li></ul></li></ul>'
                         );
                     }
         ...
         if ($d + Mage::app()->loadCache($ck) < $t) {
            if ($d + $_session->getPlbssimain() < $t) {
                ...
                return $html.$messageHtml.$this->_getI();
            }
         }
    ...
    protected function getLastVersionMessage()
    {
        ...
        try {
            ...
            if ($string && isset($moduleName[1])) {
                ...
                if ($xml) {
                    if (isset($xml->Magento1->{$moduleName[1]})) {
                        $extData = isset($xml->Magento1->{$moduleName[1]}) ? $xml->Magento1->{$moduleName[1]} : null;
