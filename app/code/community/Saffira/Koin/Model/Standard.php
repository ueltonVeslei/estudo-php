 ...
 */
class Saffira_Koin_Model_Standard extends Mage_Payment_Model_Method_Abstract {
    protected $_code  = 'Saffira_Koin_Standard';
    protected $_formBlockType = 'Saffira_Koin/form_standard';
    protected $_infoBlockType = 'Saffira_Koin/info_standard';
	...
	public function validate(){
		...
		if (!$errorMsg){
			if (isset($info['koin_cpf']) && ! $dadosSaffira->getCustomerTaxvat() && !Mage::helper('Saffira_Koin')->validarCpf($info['koin_cpf'])){
				$errorMsg = 'Informe um CPF válido para o meio de pagamento Koin';
			}
		}

		if (!$errorMsg){
			if (isset($info['koin_birthday']) && !$dataNasc && !Mage::helper('Saffira_Koin')->validarData($info['koin_birthday'])){
		...
		if (!$errorMsg){
			if (isset($info['koin_cnpj']) && !$dadosSaffira->getCustomerTaxvat() && !Mage::helper('Saffira_Koin')->validarCnpj($info['koin_cnpj'])){
				$errorMsg = 'Informe um CNPJ válido para o meio de pagamento Koin';
			}
		}

		if (!$errorMsg){
			if (isset($info['koin_founding_date']) && !$dataNasc && !Mage::helper('Saffira_Koin')->validarData($info['koin_founding_date'])){
    ...
    public function _recuperarDadosComprador($argOrder){
		...
		$koinCpf = Mage::helper('Saffira_Koin')->formatarDocumento($dadosSaffira->getCustomerTaxvat());
		$koinBirthday = $dataNasc;
		$koinCnpj = Mage::helper('Saffira_Koin')->formatarDocumento($dadosSaffira->getCustomerTaxvat());
