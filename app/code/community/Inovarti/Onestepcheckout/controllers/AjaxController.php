...
class Inovarti_Onestepcheckout_AjaxController extends Mage_Checkout_Controller_Action {
	...
	public function check_taxvatAction()
    {
        ...
        if ($taxvat) {
			...
			$cli = $readConnection->fetchOne("SELECT COUNT(`at_taxvat`.`value`) AS `total` FROM `customer_entity_varchar` AS `at_taxvat` WHERE (`at_taxvat`.`attribute_id` = '830') AND ( (at_taxvat.value = '".$taxvat."') OR (at_taxvat.value = '".preg_replace("/[^0-9]/", "", $taxvat)."'))");
