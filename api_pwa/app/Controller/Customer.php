...
class Controller_Customer extends Controller {
    ...
    protected $_excludes = ['editpassword'];

	// Recupera os dados da conta
	protected function _get() {
		$tokenInstance = new Token();
		$customerID = $tokenInstance->getIdUser($this->getData('token'));
	...
	protected function _put() {
		if ($post = (array)$this->getData('body')) {
			...
			if (isset($post['checkuser'])) {
				$hasEmail = Mage::getModel('customer/customer')
		            ->getCollection()
		            ->addAttributeToSelect('*')
		            ->addAttributeToFilter('email', $post['email'] )
	            ->getFirstItem();
