...
class Controller {
	...
	protected $_objectManager			= NULL; // Objeto de interface com magento
	...
	public function execute($token, $controller, $idUser) {
		//Registra a requisição
		$registry = new Registry();
		$res = $registry->create(['token' => $token, 'date_time_now' => date('Y-m-d H:i:s'), 'id_user' => $idUser,
			'method_controller' => $controller . '->' . $this->_method, 'data_json' => json_encode($this->_data)]);
		if($res){
			//Chama o método da controller
			return $this->_callMethod();
		}
		else{
			$response['status'] = Standard::STATUS404;
			$response['data'] = 'Não foi possível acessar o método';
		}
	}
