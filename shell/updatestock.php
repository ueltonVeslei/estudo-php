		...
		if(is_dir($strStockPath)) {
			...
			if (count($arrStockFiles) != 1) {
				...
				while ( ($arrPieces = fgetcsv($objFile, 1000, ",")) !== FALSE) {
					...
					//echo $intTotalItems . ' - ' . $mixProductSku . ' - ' . $intProductId . PHP_EOL;
					...
					if ($intProductId) {
						...
						while ($rowEntityId = mysqli_fetch_assoc($result)) {
							$productId = $rowEntityId["entity_id"];
							//echo $productId . PHP_EOL . PHP_EOL;
						}
						...
						$strQuery = "UPDATE catalog_product_entity SET updated_at = now() WHERE entity_id = " . $productId;
						$boolStatus = mysqli_query($conn,  $strQuery );

						# Update product skyhub_products
						$strQuery = "UPDATE skyhub_products SET updated_at = NULL WHERE product_id = " . $productId;
						$boolStatus = mysqli_query($conn,  $strQuery );						
						...
						// $boolStatus = mysqli_query( $strQuery, $conn );
